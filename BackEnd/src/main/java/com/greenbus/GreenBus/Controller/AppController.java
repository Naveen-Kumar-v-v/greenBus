...
public class AppController {
	...
	@PostMapping("cities")
	public List<CityEntity> postAllCities(@RequestBody List<CityEntity> city){
		return cityrepo.saveAll(city);
	}
	
	@PutMapping("cities/{id}")
	public CityEntity updateCity(@PathVariable Long id,@RequestBody CityEntity city) {
		CityEntity cityData = cityrepo.findById(id).orElseThrow(() ->  new ResourceNotFound("City Not found with this id :" + id));
	...
	public UserEntity UpdateUserDetails(@PathVariable String username , @RequestBody UserEntity user) {
		UserEntity userData = userrepo.findByUsername(username).orElseThrow(() -> new RuntimeException("user not found"));
	...
	public UserEntity resetPassword(@RequestBody UserEntity user) {
		UserEntity userData = userrepo.findByUsername(user.getUsername()).orElseThrow(() -> new RuntimeException("User not found"));
